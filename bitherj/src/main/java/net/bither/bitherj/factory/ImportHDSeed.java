...
public abstract class ImportHDSeed {
    ...
    public ImportHDSeed(ImportHDSeedType importHDSeedType,String content, List<String> worlds, SecureCharSequence password, MnemonicCode mnemonicCode) {
    ...
    public ImportHDSeed(ImportHDSeedType importHDSeedType,String content, List<String> worlds, SecureCharSequence password) {
        this.content = content;
        this.password = password;
        this.importPrivateKeyType = importHDSeedType;
        this.worlds = worlds;
        ...
        switch (importPrivateKeyType) {
            case HDSeedQRCode:
                ...
                if (hdQrCodeFlagLength > 0) {
                    ...
                    try {
                        return new HDAccount(mnemonicCode, new EncryptedData(encreyptString), password, false);
            ...
            case HDSeedPhrase://根据密语
                try {
                    byte[] mnemonicCodeSeed = mnemonicCode.toEntropy(worlds);//将助记符词表转化为原始熵值。
