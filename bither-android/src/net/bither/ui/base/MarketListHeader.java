...
public class MarketListHeader extends FrameLayout implements MarketTickerChangedObserver,
    ...
    private Animation refreshAnim = AnimationUtils.loadAnimation(getContext(),R.anim.check_light_scan);
    ...
    private void showAlert() {
        ...
        if (priceAlert != null) {
            if (priceAlert.getExchangeHigher() > 0) {
                etAlertHigh.setText(Utils.formatDoubleToMoneyString(priceAlert.getExchangeHigher()));
            }
            if (priceAlert.getExchangeLower() > 0) {
                etAlertLow.setText(Utils.formatDoubleToMoneyString(priceAlert.getExchangeLower()));
            }
        }
    }
    ...
    private class ContainerBottomPaddingHolder {
        ...
        public void shake() {
            ObjectAnimator.ofInt(this, "bottom", getBottom(), llAlert.getHeight() / 2,0).setDuration(AnimDuration * 2).start();
        }

        public void showBottom() {
            if (!isShowing()) {
                showAlert();
                ObjectAnimator.ofInt(this, "bottom", getBottom(),llAlert.getHeight()).setDuration(AnimDuration * (llAlert.getHeight() -getBottom()) / llAlert.getHeight()).start();
            }
        }
