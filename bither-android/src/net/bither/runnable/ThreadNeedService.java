...
public abstract class ThreadNeedService extends Thread {
    ...
    @Override
    public void run() {
        LogUtil.i("ansen","ThreadNeedService 是否需要开启服务:"+needService+" service:"+service);
        if (needService) {
            if (service != null) {
    ...
    public synchronized void start() {
        ...
        if (needService) {
            connected = context.bindService(new Intent(context,BlockchainService.class), connection,Context.BIND_AUTO_CREATE);
        } else {
            if (dp != null) {
                dp.setThread(this);
            }
    ...
    private ServiceConnection connection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            ThreadNeedService.this.service = ((LocalBinder) binder).getService();
            if (dp != null) {
