...
public class AddressDetailActivity extends SwipeRightFragmentActivity implements
        ...
        if (getIntent().getExtras().containsKey(BitherSetting.INTENT_REF.ADDRESS_POSITION_PASS_VALUE_TAG)) {
            ...
            if (isHDM) {
                if (addressPosition >= 0 && AddressManager.getInstance().hasHDMKeychain() &&
                    ...
                    address = AddressManager.getInstance().getHdmKeychain().getAddresses().get(addressPosition);
                }
            } else if (hasPrivateKey) {
                if (addressPosition >= 0 && AddressManager.getInstance().getPrivKeyAddresses() !=
                        null && addressPosition < AddressManager.getInstance().getPrivKeyAddresses().size()) {
                ...
                if (addressPosition >= 0 && AddressManager.getInstance().getWatchOnlyAddresses()
                        != null && addressPosition < AddressManager.getInstance().getWatchOnlyAddresses().size()) {
    ...
    private void initView() {
        findViewById(R.id.ibtn_back).setOnClickListener(new IBackClickListener(0, R.anim.slide_out_right));
    ...
    private OnClickListener optionClick = new OnClickListener() {
        ...
        public void onClick(View v) {
            LogUtil.i("ansen","AddressDetailActivity optionClicked 3");
            optionClicked();
        }
    };

    protected void optionClicked() {
        LogUtil.i("ansen","AddressDetailActivity optionClicked 1");
        Dialog dialog = null;
        if (address.isHDAccount()) {
            return;
        }
        if (address.isHDM()) {
            LogUtil.i("ansen","AddressDetailActivity optionClicked isHDM");
            new DialogHDMAddressOptions(AddressDetailActivity.this, (HDMAddress) address, true).show();
        } else if (address.hasPrivKey()) {
            LogUtil.i("ansen","AddressDetailActivity optionClicked hasPrivKey");
            dialog = new DialogAddressWithPrivateKeyOption(AddressDetailActivity.this, address);
        } else {
            LogUtil.i("ansen","AddressDetailActivity optionClicked 2");
