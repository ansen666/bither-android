...
public class HdmImportWordListActivity extends SwipeRightFragmentActivity implements TextView
    ...
    private View.OnClickListener inputClick = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            try {
                ...
                LogUtil.i("ansen","HdmImportWordListActivity 当前输入单词:"+word+" 单词总数量:"+words.size());
            } catch (IOException e) {
                e.printStackTrace();
                return;
            }
    ...
    private void complete() {
        LogUtil.i("ansen","HdmImportWordListActivity complete");
        new DialogPassword(this, new IDialogPasswordListener() {
            @Override
            public void onPasswordEntered(SecureCharSequence password) {
                LogUtil.i("ansen","HdmImportWordListActivity 当前输入密码:"+password);

                if (importHDSeedType == ImportHDSeed.ImportHDSeedType.HDSeedPhrase) {//根据密语反向hd账户
                    LogUtil.i("ansen","HdmImportWordListActivity 根据密语反向hd账户");
                    isZhTw();
                    ImportHDSeedAndroid importHDSeedAndroid = new ImportHDSeedAndroid(HdmImportWordListActivity.this, ImportHDSeed.ImportHDSeedType.HDSeedPhrase, dp, null, words, password, mnemonic);
                    importHDSeedAndroid.importHDSeed();
                } else {
                    ImportHDSeedAndroid importHDSeedAndroid = new ImportHDSeedAndroid(HdmImportWordListActivity.this, dp, words, password);
