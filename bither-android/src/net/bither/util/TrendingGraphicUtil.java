...
public class TrendingGraphicUtil {
    ...
    private static TrendingGraphicData[] trendingDatas = new TrendingGraphicData[MarketType.values().length + 1];
        ...
        BaseRunnable baseRunnable = new BaseRunnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    LogUtil.i("ansen","TrendingGraphicUtil 当前访问url:"+getExchangeTrendApi.getUrl());
                    getExchangeTrendApi.handleHttpGet();
                    JSONArray jsonArray = new JSONArray(getExchangeTrendApi.getResult());
                    ...
                    if (trendingGraphicListener != null) {
                        new Handler(Looper.getMainLooper())
                                .post(new Runnable() {
                                    @Override
                                    public void run() {
                                        trendingGraphicListener.success(trendingGraphicData);
                                    }
                                });
                    }
                } catch (Exception e) {
