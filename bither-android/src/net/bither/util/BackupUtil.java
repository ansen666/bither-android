...
public class BackupUtil {
    ...
    public static void backupColdKey(boolean checkTime,BackupListener backupListener) {
        if (AppSharedPreference.getInstance().getAppMode() != BitherjSettings.AppMode.COLD) {
        ...
        if (FileUtil.existSdCardMounted()) {
            ...
            if (checkTime) {
                ...
                if (lastBackupTime == null|| ((lastBackupTime.getTime() + ONE_WEEK_TIME) < System.currentTimeMillis()) || files.size() == 0) {
                    isBackup = true;
                }
            } else {
                isBackup = true;
            ...
            if (isBackup) {
                BackupPrivateKeyRunnable backupColdPrivateKeyRunnable = new BackupPrivateKeyRunnable(backupListener);
    ...
    public static void backupHotKey() {
        if (AppSharedPreference.getInstance().getAppMode() == BitherjSettings.AppMode.HOT) {
            BackupPrivateKeyRunnable backupColdPrivateKeyRunnable = new BackupPrivateKeyRunnable(null);
            new Thread(backupColdPrivateKeyRunnable).start();
        }
    }
    ...
    private static class BackupPrivateKeyRunnable extends BaseRunnable {
        ...
        private void backupPrivateKey() {
            ...
            LogUtil.i("ansen","BackupUtil 备份私钥的内容:"+backupString);
