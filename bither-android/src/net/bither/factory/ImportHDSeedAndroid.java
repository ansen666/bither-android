...
public class ImportHDSeedAndroid extends ImportHDSeed {
    ...
    public ImportHDSeedAndroid(Activity activity, ImportHDSeedType importHDSeedType,DialogProgress dp, String content, List<String> worlds, SecureCharSequence password, MnemonicCode mnemonicCode) {
    ...
    public void importHDSeed() {
        LogUtil.i("ansen","ImportHDSeedAndroid importHDSeed");
        new ThreadNeedService(dp, activity) {
            @Override
            public void runWithService(BlockchainService service) {
                if (service != null) {
                ...
                if (AppSharedPreference.getInstance().getAppMode() == BitherjSettings.AppMode.COLD) {
                    ...
                    LogUtil.i("ansen","ImportHDSeedAndroid 开始导入hd账户");
                    HDAccount result = importHDAccount();
                    LogUtil.i("ansen","ImportHDSeedAndroid 导入hd账户结果:"+result);
                    if (result != null) {
